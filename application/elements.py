...
class TextBox(customtkinter.CTkFrame):
    def __init__(self, *args, font: customtkinter.CTkFont = None, command = None, action_button_text: str = '', **kwargs):
        super().__init__(*args, **kwargs)
        self.grid_columnconfigure((0, 1), weight=1)
        ...
        self.copy = customtkinter.CTkButton(self, width=80, height=40, font=font, text='Copy', command=self.copy_to_clipboard)
        self.copy.pack(side='left', padx=(0, 10))
        self.paste = customtkinter.CTkButton(self, width=80, height=40, font=font, text='Paste', command=self.paste_from_clipboard)
        self.paste.pack(side='left', padx=(0, 10))

        self.action = customtkinter.CTkButton(self, width=80, height=40, font=font, text=action_button_text, command=command)
...
class Display(customtkinter.CTkFrame):
    def __init__(self, *args, boxfont: customtkinter.CTkFont = None, **kwargs):
        ...
        self.leftBox = TextBox(self, font=boxfont, fg_color = 'transparent', action_button_text='Encrypt', command=self.encrypt_text)
        ...
        self.rightBox = TextBox(self, font=boxfont, fg_color = 'transparent', action_button_text='Decrypt', command=self.decrypt_text)
